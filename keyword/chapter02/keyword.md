...
### 멱등성이란?
  ...
  **멱등성** : 연산을 여러 번 적용하더라도 결과가 달라지지 않는 성질
  ...
  - 똑같은 요청을 한 번 보내나, 백 번 보내나 서버의 상태는 똑같다

  | **메서드** | **멱등성 여부** | **설명** |
  | --- | --- | --- |
  | **GET** | **O** | 데이터를 조회만 하므로 여러 번 호출해도 서버 상태는 변하지 않습니다. |
  | **PUT** | **O** | 자원을 통째로 교체합니다. 같은 데이터로 여러 번 PUT 하면 결과는 항상 같습니다. |
  | **DELETE** | **O** | 자원을 삭제합니다. 이미 삭제된 자원을 또 삭제하라고 해도 결과적으로 "없음" 상태는 유지됩니다. |
  | **POST** | **X** | **멱등하지 않음.** 호출할 때마다 새로운 리소스가 생성될 수 있습니다. (예: 게시글 작성) |
  | **PATCH** | **△** | 구현 방식에 따라 다릅니다. 특정 필드 값을 고정값으로 바꾸면 멱등하지만, age = age + 1 처럼 계산을 포함하면 멱등하지 않습니다. |

<hr>

### HTTP 메서드 종류

| **메서드** | **주요 역할 (CRUD)** | **멱등성** | **설명** |
| --- | --- | --- | --- |
| **GET** | 조회 (Read) | **O** | 리소스를 달라고 요청합니다. 서버의 상태를 바꾸지 않는 '안전한' 메서드입니다. |
| **POST** | 등록 (Create) | **X** | 새 리소스를 생성하거나 데이터를 처리할 때 씁니다. 호출할 때마다 결과가 달라질 수 있습니다. |
| **PUT** | 전체 수정 (Update) | **O** | 리소스를 통째로 갈아치웁니다. 없으면 새로 생성하기도 합니다. |
| **PATCH** | 부분 수정 (Update) | **△** | 리소스의 특정 필드만 바꿉니다. (구현에 따라 멱등할 수도, 아닐 수도 있음) |
| **DELETE** | 삭제 (Delete) | **O** | 특정 리소스를 삭제합니다. |
    
- PUT은 "이 데이터로 전부 덮어씌워 줘" (전체 수정)
- PATCH은 "이 데이터 중 이름만 바꿔줘" (부분 수정)